...
impl AABB {
    ...
    pub fn mtv_inside(&self, aabb: &AABB) -> Option<V2> {
        let cannot_fit = aabb.width() > self.width() || aabb.height() > self.height();
        if cannot_fit {
            return None;
        }
    ...
    pub fn point_inside_aspect(point: V2, inside: V2, outside: V2) -> V2 {
        ...
        let delta = V2::new((outside.x - extents.x) / 2.0, (outside.y - extents.y) / 2.0);
    ...
    pub fn aabb_to_aspect_fit_inside(inside: V2, outside: V2) -> AABB {
        let scale = Self::scale_needed_to_fit_inside(inside, outside);
        let extents = V2::new(scale * inside.x, scale * inside.y);
        let top_left = V2::new((outside.x - extents.x) / 2.0, (outside.y - extents.y) / 2.0);
    ...
    pub fn union(a: &AABB, b: &AABB) -> AABB {
        let upper = V2::new(f32::min(a.left(), b.left()), f32::min(a.top(), b.top()));
        let lower = V2::new(
