...
impl AABBTree {
    ...
    /// let expected_result = vec![(point_ent, point_aabb), (colinear_ent, colinear_aabb)];
        ...
        B: GetStorage<Barrier>,
    {
        ...
        let should_include = shape.is_some() && pos.is_some() && include_by_barrier(*entity);
        if !should_include {
            return vec![];
        }
        ...
        self.query(&entities, &shape.aabb().translate(&pos), entity)
            .into_iter()
