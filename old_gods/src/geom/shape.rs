...
impl Shape {
  ...
  pub fn vertices(&self) -> Vec<V2> {
    match self {
      Shape::Box{ lower, upper } => {
        vec![
          *lower, V2::new(upper.x, lower.y),
          *upper, V2::new(lower.x, upper.y)
        ]
      }
  ...
  pub fn range(v: &Vec<f32>) -> (f32, f32) {
    ...
    v
      ...
      .fold(
        start,
        |(min, max), n| (f32::min(min, *n), f32::max(max, *n))
      )
  }
  ...
  pub fn mtv_apart(
    ...
    for axis in axes {
      let (my_start, my_end) =
        self
        .ranged_projection_on(this_position, axis);
      let (their_start, their_end) =
        other_shape
        .ranged_projection_on(other_position, axis);
      let does_collide =
        my_end > their_start
         && my_start < their_end;
      if !does_collide {
      ...
      if this_overlap.abs() < last_overlap.abs() {
        overlap =
          Some((this_overlap, axis));
      }
    }

    overlap
      .map(|(o, v)| v.scalar_mul(o))
  }
}
