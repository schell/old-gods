...
pub use super::geom::*;
pub use super::systems::action::{Action, FitnessStrategy, Lifespan, TakeAction};
pub use super::systems::animation::Animation;
pub use super::systems::player::{AI, Player, MaxSpeed, SuspendPlayer};
...
pub use super::systems::sprite::{Sprite, Exile, ExiledBy};
pub use super::systems::tween::{Tween, Easing, TweenParam};
...
pub fn entity_local_origin<O, S>(
  ent: Entity,
  shapes: &S,
  origins: &O
) -> V2
...
pub fn with_ent_counts<F: FnMut(), G: Fn(u32, u32)>(entities: &Entities, mut f: F, g: G) {
  let before_entity_count =
    &entities
    .join()
    .fold(0, |n, _| n + 1);
  f();
  let after_entity_count =
    &entities
    .join()
    .fold(0, |n, _| n + 1);
