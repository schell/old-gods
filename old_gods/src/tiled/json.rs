...
impl<'de> Deserialize<'de> for GlobalTileIndex {
        ...
        D: Deserializer<'de>,
    {
        ...
        let id = GlobalId(
            bits & !(FLIPPED_HORIZONTALLY_FLAG | FLIPPED_VERTICALLY_FLAG | FLIPPED_DIAGONALLY_FLAG),
        );
        Ok(GlobalTileIndex {
            id,
...
impl Tiledmap {
        ...
        for mut item in self.tilesets.iter_mut() {
            ...
            match &mut item.payload {
                ...
                TilesetPayload::Source(src) => {
                    ...
                    let tileset_dir = tileset_url.parent().expect("Tileset has no parent");
    ...
    pub fn get_tile_object_group(&self, tile_gid: &GlobalId) -> Option<ObjectGroup> {
    ...
    pub fn get_sprite_offset(&self) -> Option<V2> {
        ...
        let p: V2 = from_str(offset_value).expect("Could not deserialize sprite offset.");
