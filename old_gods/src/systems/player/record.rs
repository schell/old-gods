...
use super::super::super::components::{Name, Attributes, ZLevel};
...
impl Player {
  ...
  pub fn get_entity<'a> (&self, entities: &Entities<'a>, players: &ReadStorage<'a, Player>) -> Option<Entity> {
...
impl<'a> ToonRecord {
  ...
  pub fn read(
    map:&Tiledmap,
    object: &Object,
  ) -> Result<ToonRecord, String> {

    let attributes = Attributes::read(map, object)?;
    let name:Name =
      attributes
      .name()
      .ok_or("A player must have a name.")?;
    let _position =
      attributes
      .position()
      .ok_or("A player must have a position.")?;
    let _rendering_or_anime =
      attributes
      .rendering_or_anime()
