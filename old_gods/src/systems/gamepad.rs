...
pub fn clear_deadzone(f: f32) -> f32 {
  if f.abs() > ANALOG_DEADZONE { f } else { 0.0 }
}
...
impl PlayerController {
  ...
  fn controller_event_motion(
    this: bool,
    last: bool,
    repeat: bool,
  ) -> ControllerEventMotion {
    if this {
  ...
  pub fn query(&self, name: &ControllerEventName) -> ControllerEventMotion {
    match name {
      ControllerEventName::A => Self::controller_event_motion(
        self.this_frame.a,
        self.last_frame.a,
        false,
      ),
      ControllerEventName::B => Self::controller_event_motion(
        self.this_frame.b,
        self.last_frame.b,
        false,
      ),
      ControllerEventName::X => Self::controller_event_motion(
        self.this_frame.x,
        self.last_frame.x,
        false,
      ),
      ControllerEventName::Y => Self::controller_event_motion(
        self.this_frame.y,
        self.last_frame.y,
        false,
      ),
      ControllerEventName::Start => Self::controller_event_motion(
        self.this_frame.start,
        self.last_frame.start,
        false,
      ),
      ControllerEventName::Back => Self::controller_event_motion(
        self.this_frame.back,
        self.last_frame.back,
        false,
      ),
  ...
  fn step_gamepad(&mut self, gamepad: &Gamepad) {
    self.last_frame =
      std::mem::replace(&mut self.this_frame, ControllerState::new());
