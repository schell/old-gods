    ...
    lazy
      .exec_mut(|world| {
        ...
        let mut background_color =
          world
          .write_resource::<BackgroundColor>();
  ...
  fn load_layer_data(
    ...
    match data {
      LayerData::Tiles(tiles) => {
        Ok(
          self
            .load_tile_layer(&tiles.data, map)?
        )
      }
      LayerData::Objects(objects) => {
        if layer_name == "inventories" {
          ...
          let top_level_entities =
            inv_layer
            .into_ecs(self.world, self.z_level)?;
          ...
          .fold(
            ...
            |res, layer| {
              let mut res =
                res?;
              self
                .increment_z_by_layer(&layer);
              let mut ents =
                self
                .load_layer_data(&layer.name, &layer.layer_data, map)?;
              res.append(&mut ents);
              Ok(res)
          })
      }
      ...
      ty => {
        ...
        if let Some(gid) = gid {
          ...
          if object.text.len() > 0 {
            ...
            p.0 += self.origin;
            println!("  {:?} with attributes:{:?} and z_level:{:?}", ty, attribs, self.z_level);
            Ok(
              attribs
                .into_ecs(self.world, self.z_level)
            )
