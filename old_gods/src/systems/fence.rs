...
use super::super::geom::{AABB, AABBTree, EntityBounds, LineSegment, V2};
...
impl FenceSystem {
  pub fn run_fence(
    ...
    let last_watching:HashMap<Entity, V2> =
      fence
      .watching
      .drain()
      .collect();
    ...
    for (p1, p2) in segments {
      ...
      for EntityBounds{ entity_id, bounds } in ebs {
        ...
        let entity_center =
          AABB::from_mbr(bounds)
          .center();
        fence
          .watching
          .insert(entity, entity_center);
        ...
        if let Some(prev_center) = last_watching.get(&entity) {
          ...
          let ent_segment =
            LineSegment::new(*prev_center, entity_center);
          let intersection_point =
            fence_segment.intersection_with(ent_segment);
...
impl<'a> System<'a> for FenceSystem {
  ...
  fn run(
    ...
    // Run step fences
    for (fence_ent, step_fence, &Position(pos)) in (&entities, &mut step_fences, &positions).join() {
      self.run_fence(
        &aabb_tree,
        &entities,
        fence_ent,
