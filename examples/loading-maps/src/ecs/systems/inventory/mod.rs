...
use old_gods::{
    prelude::{
        AABBTree, Action, Exile, FitnessStrategy, Lifespan, Name, Object, OriginOffset, Player,
        PlayerControllers, Position, Rendering, Shape, SuspendPlayer, AABB, JSON, V2,
    },
    utils::clamp,
};
        ...
        let result = {
            ...
            if looting.index.is_none() {
                ...
                if navigated_items_len > 0 {
                    ...
                    println!(
                        "Switching loot navigation to other inventory - the current one is out of \
                         items."
                    );
...
impl<'a> System<'a> for InventorySystem {
    ...
    fn run(
        ...
        for (holder_ent, JSON(properties)) in (&entities, &mut jsons).join() {
                ...
                .flatten()
            {
                let inv_ent = invs.remove(&name).expect(&format!(
                    "inventory_name must reference an existing inventory: cannot find inventory \
                     named '{}'",
                    name
                ));
        ...
        for (ent, _inv, player, _) in (
            ...
            .join()
        {
            let entity_already_looting_or_being_looted = looted_inventory_ents.contains(&ent);
            looting_data
                .player_controllers
                .with_player_controller_at(player.0, |ctrl| {
                    ...
                    let wants_to_open = inv_btn_on && !entity_already_looting_or_being_looted;
                    let wants_to_close = inv_btn_on && entity_already_looting_or_being_looted;
                    if wants_to_open {
